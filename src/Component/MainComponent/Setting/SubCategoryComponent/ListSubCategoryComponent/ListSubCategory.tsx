...
const ListSubCategory = (props: any) => {
    ...
    const { SubCategoryFunction, DeleteSubCategoryFuncation,
        ActivetedDeleteSubCategoryFuncation } = Productcontext;
        ...
        try {
            ...
            if (response.Success === true) {
                if (!response.data) {
                    ...
                    await CallSubCategory(currentPage, itemsPerPage, sortBy, sortOrder, IsActive, IsDeleted);
                }
            } else {
                props.setLoading(false);
            }
    ...
    const CallSubCategory = async (cP: any, iP: number, sB: string, sO: number, Active: string, Deleted: string) => {
        props.setLoading(true);
        try {
            let jsonObject = {
            ...
            if (response.Success) {
                setTotalRecords(response.data.subCategoryCount);
                setSubCategoryList(response.data.subCategoryTypeModels);
                console.log(response.data.subCategoryTypeModels);
                props.setLoading(false);
            } else {
                props.setLoading(false);
            }
    ...
    const handleActive = async (lid: string) => {
        props.setLoading(true);
        try {
            let response = await ActivetedDeleteSubCategoryFuncation(lid);
            setId('');
            if (response.Success) {
                CallCheckuser();
    ...
    const ActivetedDelete = (cid: string) => {
        confirmAlert({
                            ...
                            <td>
                                <div className="btn-group">
                                    ...
                                    <ul className="dropdown-menu">
                                        ...
                                        <li>
                                            <button type="button" className="btn btn-outline-danger m-2" onClick={() => Delete(subcategory.subCategoryId)}
                                                style={{ display: subcategory.delete ? 'none' : 'inline-block' }}>
                                                <span className="bi bi-trash"></span> Delete
                                            </button>
                                        </li>

                                        <li>
                                            <button type="button" className="btn btn-outline-danger m-2" onClick={() => ActivetedDelete(subcategory.subCategoryId)}
                                                style={{ display: subcategory.delete ? 'inline-block' : 'none' }}>
                                                <span className="bi bi-trash"></span> Re Activate
                                            </button>
                                        </li>
                                    </ul>
                                </div>
